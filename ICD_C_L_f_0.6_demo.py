# Demonstration of ICD-C on L(f,0.6)
...
feat_dim = 4096 # Dimension of clustering feature input
# ==========
# Layer configurations
gen_dims = [feat_dim, 2048, 512, 256] # Layer configuration of generator
hid_dim = gen_dims[-1] # Dimension of embedding
...
gen_lr = 5e-5 # Learning rate of generator
disc_lr = 5e-5 # Learning rate of discriminator
...
# ==========
edge_train_list = edge_list[0: num_train] # Edge list for graphs in the training set
edge_val_list = edge_list[num_train: num_train+num_val] # Edge list for graphs in the validation set
edge_test_list = edge_list[num_train+num_val: ] # Edge list for graphs in the test set
# ==========
...
# ====================
num_train_epoch = 100 # Number of training epochs
for epoch in range(num_train_epoch):
    ...
    for q in range(p): # Randomly select p snapshots from the training set to optimize the model
        ...
        degs_ = np.diag(np.sqrt(degs))
        #vol = np.diag(np.matmul(np.matmul(np.transpose(label_onehot), adj), label_onehot))
        vol = np.matmul(np.transpose(label_onehot), degs)
        vol = np.maximum(vol, 1e-1)
        vol = 1/np.sqrt(vol)
        gnd_ind = np.matmul(label_onehot, np.diag(vol))
        gnd_ind = np.matmul(degs_, gnd_ind) # Partitioning membership (ground-truth) indicator
        ...
        gnd_tnr = torch.FloatTensor(gnd_ind).to(device) # Tensor of the partitioning membership indicator

        # =====================
        # Extract the neighbor-induced feature (i.e., Markov matrix)
        mar_tnr = get_mar_GPU(adj_tnr, num_nodes, device) # Markov matrix
        if torch.cuda.is_available():
            mar = mar_tnr.cpu().data.numpy()
        else:
            mar = mar_tnr.data.numpy()
        if feat_norm_flag:
            mar_ = m_norm(mar) # Get the normalized Markov matrix (w/ value range [-1, 1])
        else:
            mar_ = mar
        feat_norm_tnr = torch.FloatTensor(mar_).to(device)
        # ==========
    ...
    if save_flag: # If save_flag=True, save the trained model w.r.t. the current epoch
        torch.save(gen_net, 'chpt/ICD-C_gen_%s_%d.pkl' % (data_name, epoch))
        torch.save(disc_net, 'chpt/ICD-C_disc_%s_%d.pkl' % (data_name, epoch))
    ...
    # ====================
    f_output = open('res/ICD-C_%s_val.txt' % (data_name), 'a+')
    ...
    # ====================
    f_output = open('res/ICD-C_%s_test.txt' % (data_name), 'a+')
